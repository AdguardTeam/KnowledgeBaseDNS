...
1. On Windows, open Command Prompt (*Start* → *Run* → `cmd.exe`). On other systems, open your Terminal application.
...
See the [*Configuring Devices* section](/docs/adguard-home/getting-started.md#configure-devices) on the *Getting Started* page.
...
[encr]: /docs/adguard-home/encryption.md#reverse-proxy
...
1. Navigate to the directory where AdGuard Home is installed. On most Unix systems the default directory is `/opt/AdGuardHome`, but on macOS it’s `/Applications/AdGuardHome`.
