...
Queries for the `DS` query type are following the assumption based on specification for records' presence given in [RFC 4035, section
...
```none
Before:  server=/0-100.com/114.114.114.114 After:   [/0-100.com/]114.114.114.114
```
...
**BUG:** The file, just like the input in the web interface, currently doesn't accept internationalized domain names (e.g. `пример.рф` or `例子.中国`). As a workaround, convert them to Punycode (e.g. `xn--e1afmkfd.xn--p1ai` or `xn--fsqu00a.xn--fiqs8s` respectively). See [issue 2915][2915].
...
## Configuration file {#configuration-fil}
...
- `http`: Web interface configuration.
    ...
    - `pprof`: Profiling HTTP handler configuration. See
...
- `auth_attempts`: Maximum number of failed login attempts a user can do before being blocked. The entire blocking logic is disabled if it equals to 0.
- `block_auth_min`: The duration of blocking period. The entire blocking logic is disabled if it equals to 0.
...
- `theme`: The UI theme. The possible values are:
  `auto`, `dark`, `light`.
- `dns`: DNS configuration section.
    - **General settings**
        - `bind_hosts`: IP addresses on which to serve DNS queries. For each network interface there can only be one IP address of each IP version. Examples of valid configurations:
    ...
    - **Protection settings  (before v0.107.37)**
        - `protection_enabled`: Whether any kind of filtering and protection should be performed. It doesn't affect the rules with `$dnsrewrite` modifier and other rewrites, including those taken from the operating system's hosts file.
    ...
    - **Query log settings**
        - query log settings are part of `querylog` object.
    ...
    - **Upstream DNS servers settings:**
        ...
        - `local_ptr_upstreams`: List of upstream DNS servers to resolve PTR requests for addresses inside locally-served networks. If empty, AdGuard Home will automatically try to get local resolvers from the OS.

      Domain-specific upstreams are validated for being a valid ARPA domain pointing to a locally-served network.

        - `upstream_mode`: The mode describes the logic through which the upstreams will be used. The possible values are:
        ...
        - `pending_requests`: The configuration for cache poisoning attacks protection.
    ...
    - **ECS settings**
        - `edns_client_subnet`: Controls EDNS Client Subnet behavior.
    ...
    - **Other settings**
        ...
        - `safebrowsing_cache_size` (**before v0.107.37**): Safe Browsing cache size, in bytes. <!--TODO(f.setrakov): !! before? -->
        - `safesearch_cache_size` (**before v0.107.37**): Safe Search cache size, in bytes.
      ...
      **NOTE:** `serve_plain_dns` cannot currently be set to `false` unless one or more encrypted protocols (DNS-over-HTTPS, DNS-over-TLS, etc.) is enabled.
...
- `filtering`: Filtering settings section:
    ...
    - `filtering_enabled`: Whether filtering of DNS requests based on rule lists should be performed.
    - `blocking_mode`: Specifies how to block DNS requests. The possible values are:
...
- `querylog`: Query log settings section:
    - `enabled`: Query log status.
    - `file_enabled`: Write query logs to a file.
    - `interval`: Time interval for query log files
...
- `statistics`: Statistics settings section:
...
- `tls`: HTTPS/DoH/DoQ/DoT settings.
    ...
    - `server_name`: The hostname of your server. If set, it is used to detect ClientIDs (using the ServerName field of ClientHello messages), respond to [Discovery of Designated Resolvers (DDR)][DDR] queries, and perform additional connection validations. If not set, these features are disabled.
    ...
    - `port_dnscrypt`: The DNSCrypt port. If `0`, DNSCrypt is disabled. See the “[DNSCrypt] [dnscrypt-wiki]” page for more information and examples.
    - `dnscrypt_config_file`: The path to the DNSCrypt configuration file.
    **Must** be set if `port_dnscrypt` is not `0`. See the [`dnscrypt`] utility documentation for examples of configuration generation.
...
- `os`: Operating system related settings.
...
- **Log settings**
    - `log`: Log settings.
        ...
        - `local_time`: If `true`, the time used for formatting the timestamps in is the computer's local time.
        ...
        - `max_age`: MaxAge is the maximum number of days to retain old log files.
