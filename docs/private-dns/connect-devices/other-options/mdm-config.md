...
On iOS, AdGuard DNS uses DNS proxy to enforce DNS policies on managed devices. To enable this, creates a `mobileconfig` for the device and deploys it via MDM. A minimal profile consists of a top-level payload with a single nested payload:
...
<dict>
    ...
    <string>AdGuard DNS DNSProxy 1</string>
...
| **PayloadDisplayName**  | `AdGuard DNS DNSProxy 1`                          | Name shown to the user in Settings and to the admin in MDM.                 |
...
| **PayloadDisplayName**       | `AdGuard DNS DNSProxy 1`                       | This is the name displayed for this specific part of the profile. It may be the same as the `PayloadUUID` or different. The name is shown to the user under Settings → General → VPN & Device Management → DNS.                                             |
