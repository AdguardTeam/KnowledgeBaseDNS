...
| **App lock** (`lock_type`)        | Choice   | No           | **iOS:** block, none | Locks the AdGuard DNS app, which prevents device user from accessing the app. | App enters managed mode; app locks if the value is set to `block`.          | Unlocks the app if value is changed to `none`; blocks the app if value is changed to `block`. | App is not locked, user can access the app and its settings.                      |
