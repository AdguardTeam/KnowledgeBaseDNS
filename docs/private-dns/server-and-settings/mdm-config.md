...
| **Device name** (`device_name`)   | String   | No           | Any device name                                                                         | Sets the device's display name in the AdGuard DNS control panel.                                    | App enters managed mode; name used automatically if not set via Setup ID; field is locked.              | Nothing happens.                                                                        | User can enter manually; app may suggest name via Setup ID. |
| **DNS Protocol** (`dns_protocol`) | Choice   | No           | **Android:** doh, dot, doq. **iOS:** doh_native, dot_native, doh_vpn, dot_vpn, doq_vpn. | Determines which encrypted DNS protocol is used (Note: DoQ not compatible with Native mode on iOS). | App enters managed mode; specified protocol applied by default; selection in settings locked.           | App switches to the new protocol; reconnects automatically if DNS protection is active. | User can select manually in settings.                       |
