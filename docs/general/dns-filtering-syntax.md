...
multiple filter lists regardless of their type, we provide a tool that can be used to compile blocklists. We called it [Hostlist compiler][hlc] and we use it ourselves to create [AdGuard DNS filter][sdn].
...
*  You may want to use multiple modifiers in a rule.  Separate them by commas in this case:
   ...
   And finally, `dnstype=A` is the [`dnstype`](#dnstype) modifier with its value, `A`.
...
The `client` modifier can only be used in AdGuard Home. It allows specifying clients this rule is applied to. There are two main ways to identify a client:
...
spell your modifier as `$client='My Client'` as opposed to `$client=my-client`.
...
The `dnsrewrite` response modifier allows replacing the content of the response to the DNS request for the matching hosts. You can use this modifier both for rules in Private DNS and in AdGuard Home. However, in the first case it will work only in custom rules, while in the second —  in all rules.
