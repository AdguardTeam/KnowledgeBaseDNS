...
Responses to all requests for a host matching a `dnsrewrite` rule will be replaced. The replacement response's answer section will only contain RRs that match the request's query type and, possibly, CNAME RRs. Note that this means that responses to some requests may become empty (`NODATA`) if the host matches a `dnsrewrite` rule.
