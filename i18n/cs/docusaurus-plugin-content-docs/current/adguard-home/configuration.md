...
- `doh`: DNS-over-HTTPS configuration. Vyznačuje se těmito vlastnostmi:
...
Web user info. If set to an empty list (`[]`), authentication is disabled. Vyznačuje se těmito vlastnostmi:
...
The UI theme. Přípustné hodnoty:
...
The `dns` object configures the DNS server. Vyznačuje se těmito vlastnostmi:
...
- `pending_requests`: The configuration for cache poisoning attacks protection. Vyznačuje se těmito vlastnostmi:
...
- `edns_client_subnet`: Controls EDNS Client Subnet behavior. Vyznačuje se těmito vlastnostmi:
...
The `filtering` object configures filtering settings. Vyznačuje se těmito vlastnostmi:
...
- `blocking_mode`: Specifies how to block DNS requests. Přípustné hodnoty:
...
- `safe_search`: _Safe search_ settings section. Vyznačuje se těmito vlastnostmi:
...
- `blocked_services`: Blocked services settings section. Vyznačuje se těmito vlastnostmi:
...
The `querylog` object configures _Query log_ settings. Vyznačuje se těmito vlastnostmi:
...
The `statistics` object configures statistics settings. Vyznačuje se těmito vlastnostmi:
...
Built-in DHCP server configuration. See also the [DHCP][DHCPv4] article. Vyznačuje se těmito vlastnostmi:

- `enabled`: DHCP server status.

- `interface_name`: Network interface name (`eth0`, `en0`, and so on).

- `dhcpv4`: DHCPv4 settings. Vyznačuje se těmito vlastnostmi:
...
- `dhcpv6`: DHCPv6 settings. Vyznačuje se těmito vlastnostmi:
...
HTTPS/DoH/DoQ/DoT settings. Vyznačuje se těmito vlastnostmi:
