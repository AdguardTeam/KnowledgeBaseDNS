...
V systému iOS používá AdGuard DNS proxy DNS k prosazování zásad DNS a zůstává **vždy zapnutý** pro sledovaná zařízení. Chcete-li toto povolit, vytvořte pro zařízení `mobileconfig` a nasaďte jej prostřednictvím MDM. For MAC’s `dns_protocol` parameter, use `doh_dnsproxy`, `dot_dnsproxy`, or `doq_dnsproxy`.
