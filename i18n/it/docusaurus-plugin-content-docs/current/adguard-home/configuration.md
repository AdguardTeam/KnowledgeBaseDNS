...
- `doh`: DNS-over-HTTPS configuration. Ha le seguenti proprietà:
...
Web user info. If set to an empty list (`[]`), authentication is disabled. Ha le seguenti proprietà:
...
The `dns` object configures the DNS server. Ha le seguenti proprietà:
...
- `upstream_mode`: The mode describes the logic through which the upstreams will be used. Valori possibili:
...
- `pending_requests`: The configuration for cache poisoning attacks protection. Ha le seguenti proprietà:
...
- `edns_client_subnet`: Controls EDNS Client Subnet behavior. Ha le seguenti proprietà:
...
The `filtering` object configures filtering settings. Ha le seguenti proprietà:
...
- `blocking_mode`: Specifies how to block DNS requests. Valori possibili:
...
- `safe_search`: _Safe search_ settings section. Ha le seguenti proprietà:
...
- `blocked_services`: Blocked services settings section. Ha le seguenti proprietà:
...
The `querylog` object configures _Query log_ settings. Ha le seguenti proprietà:
...
The `statistics` object configures statistics settings. Ha le seguenti proprietà:
...
Built-in DHCP server configuration. See also the [DHCP][DHCPv4] article. Ha le seguenti proprietà:

- `enabled`: DHCP server status.

- `interface_name`: Network interface name (`eth0`, `en0`, and so on).

- `dhcpv4`: DHCPv4 settings. Ha le seguenti proprietà:
...
- `dhcpv6`: DHCPv6 settings. Ha le seguenti proprietà:
...
HTTPS/DoH/DoQ/DoT settings. Ha le seguenti proprietà:
