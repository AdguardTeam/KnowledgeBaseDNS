...
- `doh`: DNS-over-HTTPS configuration. Tiene las siguientes propiedades:
...
Web user info. If set to an empty list (`[]`), authentication is disabled. Tiene las siguientes propiedades:
...
The `dns` object configures the DNS server. Tiene las siguientes propiedades:
...
- `upstream_mode`: The mode describes the logic through which the upstreams will be used. Valores posibles:
...
- `pending_requests`: The configuration for cache poisoning attacks protection. Tiene las siguientes propiedades:
...
- `edns_client_subnet`: Controls EDNS Client Subnet behavior. Tiene las siguientes propiedades:
...
The `filtering` object configures filtering settings. Tiene las siguientes propiedades:
...
- `blocking_mode`: Specifies how to block DNS requests. Valores posibles:
...
- `safe_search`: _Safe search_ settings section. Tiene las siguientes propiedades:
...
- `blocked_services`: Blocked services settings section. Tiene las siguientes propiedades:
...
The `querylog` object configures _Query log_ settings. Tiene las siguientes propiedades:
...
The `statistics` object configures statistics settings. Tiene las siguientes propiedades:
...
Built-in DHCP server configuration. See also the [DHCP][DHCPv4] article. Tiene las siguientes propiedades:

- `enabled`: DHCP server status.

- `interface_name`: Network interface name (`eth0`, `en0`, and so on).

- `dhcpv4`: DHCPv4 settings. Tiene las siguientes propiedades:
...
- `dhcpv6`: DHCPv6 settings. Tiene las siguientes propiedades:
...
HTTPS/DoH/DoQ/DoT settings. Tiene las siguientes propiedades:
