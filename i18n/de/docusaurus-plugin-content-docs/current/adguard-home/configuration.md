...
- `doh`: DNS-over-HTTPS configuration. Es hat folgende Eigenschaften:
...
Web user info. If set to an empty list (`[]`), authentication is disabled. Es hat folgende Eigenschaften:
...
The `dns` object configures the DNS server. Es hat folgende Eigenschaften:
...
- `upstream_mode`: The mode describes the logic through which the upstreams will be used. Mögliche Werte:
...
- `pending_requests`: The configuration for cache poisoning attacks protection. Es hat folgende Eigenschaften:
...
- `edns_client_subnet`: Controls EDNS Client Subnet behavior. Es hat folgende Eigenschaften:
...
The `filtering` object configures filtering settings. Es hat folgende Eigenschaften:
...
- `blocking_mode`: Specifies how to block DNS requests. Mögliche Werte:
...
- `safe_search`: _Safe search_ settings section. Es hat folgende Eigenschaften:
...
- `blocked_services`: Blocked services settings section. Es hat folgende Eigenschaften:
...
The `querylog` object configures _Query log_ settings. Es hat folgende Eigenschaften:
...
The `statistics` object configures statistics settings. Es hat folgende Eigenschaften:
...
Built-in DHCP server configuration. See also the [DHCP][DHCPv4] article. Es hat folgende Eigenschaften:

- `enabled`: DHCP server status.

- `interface_name`: Network interface name (`eth0`, `en0`, and so on).

- `dhcpv4`: DHCPv4 settings. Es hat folgende Eigenschaften:
...
- `dhcpv6`: DHCPv6 settings. Es hat folgende Eigenschaften:
...
HTTPS/DoH/DoQ/DoT settings. Es hat folgende Eigenschaften:
