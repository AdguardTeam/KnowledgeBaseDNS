...
- `doh`: DNS-over-HTTPS configuration. Ele tem as seguintes propriedades:
...
Web user info. If set to an empty list (`[]`), authentication is disabled. Ele tem as seguintes propriedades:
...
The UI theme. Valores possíveis:
...
The `dns` object configures the DNS server. Ele tem as seguintes propriedades:
...
- `upstream_mode`: The mode describes the logic through which the upstreams will be used. Valores possíveis:
...
- `pending_requests`: The configuration for cache poisoning attacks protection. Ele tem as seguintes propriedades:
...
- `edns_client_subnet`: Controls EDNS Client Subnet behavior. Ele tem as seguintes propriedades:
...
The `filtering` object configures filtering settings. Ele tem as seguintes propriedades:
...
- `blocking_mode`: Specifies how to block DNS requests. Valores possíveis:
...
- `safe_search`: _Safe search_ settings section. Ele tem as seguintes propriedades:
...
- `blocked_services`: Blocked services settings section. Ele tem as seguintes propriedades:
...
The `querylog` object configures _Query log_ settings. Ele tem as seguintes propriedades:
...
The `statistics` object configures statistics settings. Ele tem as seguintes propriedades:
...
Built-in DHCP server configuration. See also the [DHCP][DHCPv4] article. Ele tem as seguintes propriedades:

- `enabled`: DHCP server status.

- `interface_name`: Network interface name (`eth0`, `en0`, and so on).

- `dhcpv4`: DHCPv4 settings. Ele tem as seguintes propriedades:
...
- `dhcpv6`: DHCPv6 settings. Ele tem as seguintes propriedades:
...
HTTPS/DoH/DoQ/DoT settings. Ele tem as seguintes propriedades:
