...
Блокирует доступ к контенту для взрослых, прокси & VPN и к сайтам со смешанным содержимым
...
Обычные DNS-серверы, которые защищают от фишинга и шпионских программ. Включает список блокировки, валидацию DNSSEC и другие функции безопасности.
...
[BlahDNS](https://blahdns.com/) Небольшой любительский DNS-проект. Без логирования, Ethereum Name Service, DNSSEC & Фильтруемая реклама, трекеры, вредоносное ПО
...
[DNS for Family](https://dnsforfamily.com/) блокирует сайты для взрослых. Помогает пользоваться интернетом безопасно, не беспокоясь о том, что можно попасть на вредоносный сайт.
...
[IIJ.JP](https://public.dns.iij.jp/) — это публичный DNS-сервис, управляемый компанией Internet Initiative Japan. Также он блокирует контент с изображением жестокого обращения с детьми.
...
[DNSPod Public DNS+](https://www.dnspod.com/) — это защищающий конфиденциальность DNS-провайдер с годами опыта в развитии DNS-сервисов, который стремится обеспечить пользователей быстрым, точным и стабильным рекурсивным сервисом разрешения имён.
...
[DeCloudUs DNS](https://decloudus.com/) Безопасный, конфиденциальный DNS-резолвер с открытым исходным кодом, защитой от вредоносного ПО, блокировкой рекламы и без логирования. Degoogle and Ungoogle your phone, tablet, and computer.
...
[DNS Forge](https://dnsforge.de/) — это запасной DNS-резолвер с блокировщиком рекламы без логирования от провайдера [adminforge](https://adminforge.de/).
...
[puntCAT](http://www.servidordenoms.cat/) физически расположен рядом с Барселоной, Испания. puntCAT предлагает публичный DNS-сервис, бесплатный, безопасный и уважающий вашу конфиденциальность.
...
Семейный режим включает защиту «Безопасных» резолверов и блокирует сайты для взрослых.
...
Блокирует больше рекламы и вредоносного ПО, чем AdGuard DNS благодаря более продвинутому синтаксису, но _ и блокирует _ и большинство имиджбордов. Логирование используется, чтобы улучшить используемые им фильтры (например, для разблокирования сайтов, которые не должны были блокироваться) и определить _ время для системного обновления сервера. For additional important info, see the link above.
