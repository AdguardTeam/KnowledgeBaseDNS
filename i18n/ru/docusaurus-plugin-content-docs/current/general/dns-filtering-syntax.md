...
Модификатор `client` можно использовать только в AdGuard Home и AdGuard DNS.
...
Модификатор `respgeo` можно использовать только в AdGuard DNS.

:::

Модификатор `respgeo` позволяет применять правила фильтрации в зависимости от страны или ASN IP-адреса, возвращаемого в DNS-ответе. Он анализирует исключительно **целевой IP** — тот, в который разрешается домен. При этом IP-адрес, страна или ASN самого пользователя, его устройства или DNS-клиента **не** проверяются.

##### Блокировка по стране ответа

Значение модификатора должно быть представлено в виде двухбуквенного кода страны в формате ISO 3166-1 alpha-2. Также можно использовать `--` для сопоставления ответов, в которых страну определить не удалось.

**Примеры:**

- `||*^$respgeo=US`: блокирует домены, если IP-адрес в DNS-ответе связан с Соединенными Штатами.
- `||*^$respgeo=FR|DE`: блокирует домены, если IP-адрес в DNS-ответе связан с Францией или Германией.
- `||*^$respgeo=--`: блокирует домены, если страна IP-адреса в DNS-ответе неизвестна.
- `||*^$respgeo=~--`: блокирует домены, если страна, к которой относится IP-адрес в DNS-ответе, известна.
- `@@||whitehouse.gov^`: разрешает `whitehouse.gov`, даже если он заблокирован wildcard-правилом с модификатором `respgeo`.
- `||*^$respgeo=US`: разрешает `example.org`, если IP-адрес в DNS-ответе связан с Соединенными Штатами.
- `||whitehouse.gov^$respgeo=US`: блокирует `whitehouse.gov` только если IP-адрес в DNS-ответе связан с Соединёнными Штатами.
- В этом примере:
  ...
  `@@||whitehouse.gov^$respgeo=US` **не** разрешит `whitehouse.gov`, поскольку первое правило блокирует запрос на основе данных запроса, а второе пытается разрешить его на основе данных ответа.

Для инвертирования условия можно использовать `~`:

- `||*^$respgeo=~DE`: блокирует домены, если IP-адрес в DNS-ответе **не** связан с Германией.

**Ограничения**

Модификатор `respgeo` использует один вычисленный IP-адрес и страну в соответствии с текущей логикой *Журнала запросов*. Если домен разрешается в несколько IP-адресов или стран, AdGuard DNS не анализирует все возвращённые IP-адреса.

Поскольку многие домены используют CDN, балансировку нагрузки или географически распределенную инфраструктуру, определяемая страна может меняться со временем.

Если страну не удаётся определить, условие GeoIP не будет выполнено. Используйте `respgeo=--`, чтобы сопоставлять ответы с неизвестной страной.

Правила с модификатором `respgeo` отображаются в * Журнале запросов* как обычные правила.

##### Блокировка по ASN

Модификатор `respgeo` также может использоваться для применения правил на основе ASN IP-адреса, возвращаемого в DNS-ответе.

ASN расшифровывается как **Autonomous System Number** — номер автономной системы.  Он идентифицирует автономную систему — сеть, управляемую интернет-провайдером, хостинг-провайдером, облачным провайдером, компанией или другой организацией.

Этот модификатор проверяет **целевой ASN** — ASN, связанный с IP-адресом, на который разрешается домен. Он **не** проверяет ASN пользователя, устройства или DNS-клиента.

Значением модификатора должен быть ASN в формате `AS<number>`, например `AS15169`.

**Примеры:**

- `||*^$respgeo=AS15169`: блокирует домены, если IP-адрес в DNS-ответе принадлежит ASN AS15169.
- `||*^$respgeo=AS15169|AS8075`: блокирует домены, если IP-адрес в DNS-ответе принадлежит ASN AS15169 или AS8075.
- `||*^$respgeo=AS--`: блокирует домены, если автономный номер IP-адреса в DNS-ответе неизвестен.
- `||*^$respgeo=~AS--`: блокирует домены, если известен ASN IP-адреса в DNS-ответе.
- `@@||google.com^$respgeo=AS15169`: разрешает `google.com`, если IP-адрес в DNS-ответе принадлежит ASN AS15169.
- `||google.com^$respgeo=AS15169`: блокирует `google.com` только если IP-адрес в DNS-ответе принадлежит ASN AS15169.
- В этом примере:
  ...
  `@@||google.com^$respgeo=AS15169` **не** разрешит `google.com`, потому что первое правило блокирует запрос, анализируя данные запроса, в то время как второе пытается разрешить его, анализируя ответ.

Для инвертирования условия можно использовать `~`:

- `||*^$respgeo=~AS15169`: блокирует домены, если IP-адрес в ответе DNS **не** принадлежит ASN AS15169.

**Ограничения**

Модификатор `respgeo` использует один вычисленный IP-адрес и ASN в соответствии с текущей логикой *Журнала запросов*. Если домен разрешается в несколько IP-адресов или ASN, AdGuard DNS не анализирует все возвращённые ASN.

Крупные CDN, облачные или хостинговые ASN могут содержать множество несвязанных веб-сайтов. Таким образом, блокировка ASN может затронуть больше доменов, чем ожидается.

Если ASN не может быть определён, условие ASN не будет выполнено. Используйте `respgeo=AS--`, чтобы сопоставить ответы с неизвестным ASN.

ASN не всегда соответствует конкретной компании, продукту или услуге. Он лишь определяет сеть, связанную с разрешённым IP-адресом.
