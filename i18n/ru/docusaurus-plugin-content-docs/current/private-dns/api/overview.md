...
| **client_id**               | Да          | Your registered OAuth2 client identifier |
| **redirect_uri**            | Да          | Where to redirect after authorization    |
| **state**                   | Да          | Random string to prevent CSRF            |
| **code_challenge**          | Да          | BASE64URL(SHA256(code_verifier))         |
| **code_challenge_method** | Да          | Must be `S256`                           |
...
| Параметр          | Обязательно | Описание                                       |
|:----------------- |:----------- |:---------------------------------------------- |
| **grant_type**    | Да          | Must be `authorization_code`                   |
| **code**          | Да          | Authorization code received in Step 2          |
| **client_id**     | Да          | Your registered OAuth2 client identifier       |
| **code_verifier** | Да          | The original code verifier generated in Step 1 |
| **redirect_uri**  | Да          | Must match the `redirect_uri` used in Step 2   |
