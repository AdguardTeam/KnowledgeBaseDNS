...
- `doh`: DNS-over-HTTPS configuration. Он обладает следующими свойствами:
...
Web user info. If set to an empty list (`[]`), authentication is disabled. Он обладает следующими свойствами:
...
The UI theme. Возможные значения:
...
The `dns` object configures the DNS server. Он обладает следующими свойствами:
...
- `upstream_mode`: The mode describes the logic through which the upstreams will be used. Возможные значения:
...
- `pending_requests`: The configuration for cache poisoning attacks protection. Он обладает следующими свойствами:
...
- `edns_client_subnet`: Controls EDNS Client Subnet behavior. Он обладает следующими свойствами:
...
The `filtering` object configures filtering settings. Он обладает следующими свойствами:
...
- `blocking_mode`: Specifies how to block DNS requests. Возможные значения:
...
- `safe_search`: _Safe search_ settings section. Он обладает следующими свойствами:
...
- `blocked_services`: Blocked services settings section. Он обладает следующими свойствами:
...
The `querylog` object configures _Query log_ settings. Он обладает следующими свойствами:
...
The `statistics` object configures statistics settings. Он обладает следующими свойствами:
...
Built-in DHCP server configuration. See also the [DHCP][DHCPv4] article. Он обладает следующими свойствами:

- `enabled`: DHCP server status.

- `interface_name`: Network interface name (`eth0`, `en0`, and so on).

- `dhcpv4`: DHCPv4 settings. Он обладает следующими свойствами:
...
- `dhcpv6`: DHCPv6 settings. Он обладает следующими свойствами:
...
HTTPS/DoH/DoQ/DoT settings. Он обладает следующими свойствами:
