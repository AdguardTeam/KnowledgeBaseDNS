...
- `doh`: DNS-over-HTTPS configuration. Aşağıdaki özelliklere sahiptir:
...
Web user info. If set to an empty list (`[]`), authentication is disabled. Aşağıdaki özelliklere sahiptir:
...
The `dns` object configures the DNS server. Aşağıdaki özelliklere sahiptir:
...
- `upstream_mode`: The mode describes the logic through which the upstreams will be used. Olası değerler:
...
- `pending_requests`: The configuration for cache poisoning attacks protection. Aşağıdaki özelliklere sahiptir:
...
- `edns_client_subnet`: Controls EDNS Client Subnet behavior. Aşağıdaki özelliklere sahiptir:
...
The `filtering` object configures filtering settings. Aşağıdaki özelliklere sahiptir:
...
- `blocking_mode`: Specifies how to block DNS requests. Olası değerler:
...
- `safe_search`: _Safe search_ settings section. Aşağıdaki özelliklere sahiptir:
...
- `blocked_services`: Blocked services settings section. Aşağıdaki özelliklere sahiptir:
...
The `querylog` object configures _Query log_ settings. Aşağıdaki özelliklere sahiptir:
...
The `statistics` object configures statistics settings. Aşağıdaki özelliklere sahiptir:
...
Built-in DHCP server configuration. See also the [DHCP][DHCPv4] article. Aşağıdaki özelliklere sahiptir:

- `enabled`: DHCP server status.

- `interface_name`: Network interface name (`eth0`, `en0`, and so on).

- `dhcpv4`: DHCPv4 settings. Aşağıdaki özelliklere sahiptir:
...
- `dhcpv6`: DHCPv6 settings. Aşağıdaki özelliklere sahiptir:
...
HTTPS/DoH/DoQ/DoT settings. Aşağıdaki özelliklere sahiptir:
