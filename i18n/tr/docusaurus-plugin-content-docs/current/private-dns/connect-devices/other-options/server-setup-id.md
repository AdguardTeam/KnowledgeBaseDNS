...
If you’re configuring AdGuard on multiple managed devices, consider using managed app configuration to automatically provide the server setup ID to each app. [Daha fazla bilgi edinin](/private-dns/connect-devices/other-options/mdm-config.md).
...
Tamamlandı! You are now connected, and the device will be automatically created in the Dashboard.
