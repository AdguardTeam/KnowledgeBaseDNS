...
AdGuard DNS aboneleri, varsayılan `d.adguard-dns.com` yerine kendi alan adlarını (örneğin `dns.example.com`) ayarlayabilirler. Bu özellik, AdGuard DNS tarafından sağlanan tam reklam engelleme ve gizlilik özelliklerini korurken kurumsal güvenlik politikalarıyla sorunsuz entegrasyon sağlayarak işletmelerin DNS korumasını altyapılarıyla uyumlu hâle getirmelerine yardımcı olmayı amaçlamaktadır.
