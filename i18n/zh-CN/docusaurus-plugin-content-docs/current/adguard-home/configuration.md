...
- `doh`: DNS-over-HTTPS configuration. 它包含以下属性：
...
Web user info. If set to an empty list (`[]`), authentication is disabled. 它包含以下属性：
...
The `dns` object configures the DNS server. 它包含以下属性：
...
- `upstream_mode`: The mode describes the logic through which the upstreams will be used. 可能的选项值如下：
...
- `pending_requests`: The configuration for cache poisoning attacks protection. 它包含以下属性：
...
- `edns_client_subnet`: Controls EDNS Client Subnet behavior. 它包含以下属性：
...
The `filtering` object configures filtering settings. 它包含以下属性：
...
- `blocking_mode`: Specifies how to block DNS requests. 可能的选项值如下：
...
- `safe_search`: _Safe search_ settings section. 它包含以下属性：
...
- `blocked_services`: Blocked services settings section. 它包含以下属性：
...
The `querylog` object configures _Query log_ settings. 它包含以下属性：
...
The `statistics` object configures statistics settings. 它包含以下属性：
...
Built-in DHCP server configuration. See also the [DHCP][DHCPv4] article. 它包含以下属性：

- `enabled`: DHCP server status.

- `interface_name`: Network interface name (`eth0`, `en0`, and so on).

- `dhcpv4`: DHCPv4 settings. 它包含以下属性：
...
- `dhcpv6`: DHCPv6 settings. 它包含以下属性：
...
HTTPS/DoH/DoQ/DoT settings. 它包含以下属性：
