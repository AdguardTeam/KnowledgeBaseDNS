...
- `doh`: DNS-over-HTTPS configuration. Il a les propriétés suivantes :
...
Web user info. If set to an empty list (`[]`), authentication is disabled. Il a les propriétés suivantes :
...
The UI theme. Valeurs possibles :
...
The `dns` object configures the DNS server. Il a les propriétés suivantes :
...
- `upstream_mode`: The mode describes the logic through which the upstreams will be used. Valeurs possibles :
...
- `pending_requests`: The configuration for cache poisoning attacks protection. Il a les propriétés suivantes :
...
- `edns_client_subnet`: Controls EDNS Client Subnet behavior. Il a les propriétés suivantes :
...
The `filtering` object configures filtering settings. Il a les propriétés suivantes :
...
- `blocking_mode`: Specifies how to block DNS requests. Valeurs possibles :
...
- `safe_search`: _Safe search_ settings section. Il a les propriétés suivantes :
...
- `blocked_services`: Blocked services settings section. Il a les propriétés suivantes :
...
The `querylog` object configures _Query log_ settings. Il a les propriétés suivantes :
...
The `statistics` object configures statistics settings. Il a les propriétés suivantes :
...
Built-in DHCP server configuration. See also the [DHCP][DHCPv4] article. Il a les propriétés suivantes :

- `enabled`: DHCP server status.

- `interface_name`: Network interface name (`eth0`, `en0`, and so on).

- `dhcpv4`: DHCPv4 settings. Il a les propriétés suivantes :
...
- `dhcpv6`: DHCPv6 settings. Il a les propriétés suivantes :
...
HTTPS/DoH/DoQ/DoT settings. Il a les propriétés suivantes :
