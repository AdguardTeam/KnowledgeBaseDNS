...
På iOS bruger AdGuard DNS en DNS-proxy til at håndhæve DNS-politikker og forblive **altid slået til** på overvågede enheder. For at aktivere dette, opret en `mobileconfig` til enheden og distribuér den via MDM. For MAC’s `dns_protocol` parameter, use `doh_dnsproxy`, `dot_dnsproxy`, or `doq_dnsproxy`.
