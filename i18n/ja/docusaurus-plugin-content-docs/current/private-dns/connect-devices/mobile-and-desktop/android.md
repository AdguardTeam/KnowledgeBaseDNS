...
1. AdGuard DNS アプリを [Google Play からダウンロード](https://agrd.io/android_dns)します。
...
5. 「_AdGuardアプリを使って接続_」セクションを開きます。
   ...
   ![AdGuard アプリの使用 \*border](https://cdn.adtidy.org/content/release_notes/dns/dns-app-v1-0/UseAdGuardApps-ja.png)

6. 「AdGuard DNS アプリを使う」セクションで、以下の設定方法のいずれかを選択します：
...
7. 「_続行する_」 → 「_接続する_」をタップします。

   ![Connect \*mobile_border](https://cdn.adtidy.org/content/release_notes/dns/dns-app-v1-0/Connect.png)

8. VPNベースのプロトコルを使用する場合、保護を有効にする前にVPNプロファイルの作成を許可する必要があります。 これを行うには、アプリのホームページで「_有効にする_」をクリックし、アプリ内の指示に従ってください。

9. 完了です! これで、お使いのデバイスは正常に AdGuard DNS に接続されました。 保護機能を開始したいときは、毎回「有効にする」をタップしてください。

   ![有効にする \*mobile_border](https://cdn.adtidy.org/content/release_notes/dns/dns-app-v1-0/protect-ja.png)
...
All done! これで、お使いのデバイスは正常に AdGuard DNS に接続されました。
...
4. 「保存」をタップします。
   ...
   完了です！ Your device is successfully connected to AdGuard DNS.
